...
- `quiz-configuration-store.js`: keep Immer form actions; `saveQuiz` writes `quizzes/{id}` with `ownerId`, `name`, `answerMode`, `teams`, `questionIds` (pointers, not copies).
- Fix bug 04 — pass `quizName` into `validateNewQuiz` at `quiz-configuration-store.js:71`.
