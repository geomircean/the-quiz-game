  ...
  return (
    <div className="flex min-h-screen flex-col flex-wrap items-center justify-around gap-1">
      <div className="flex flex-row gap-5 justify-evenly">
        <Button onClick={() => router.push('/admin/new-quiz-configuration')}>Configure New Quiz</Button>
        <Button disabled onClick={() => router.push('/admin/new-random-quiz')}>Generate Random Quiz</Button>
      </div>
      <div className="flex flex-row gap-2">
        <Button> Add Question To Your Database</Button>
        <Button onClick={() => router.push('/admin/questions-list')}>View Questions List</Button>
      </div>
      <div>
